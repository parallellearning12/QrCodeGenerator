...
def index():
    if request.method == 'POST':
        ...
        return redirect(url_for('success', filename=f"{file_id}.zip", count=end - start + 1))
...
def success():
    ...
    return render_template('success.html', filename=filename, count=count)
...
def download(filename):
    ...
    return send_file(file_path, mimetype='application/zip', as_attachment=True, download_name='qr_codes.zip')
